...
public class BezierPointEditor : Editor {
	...
	public override void OnInspectorGUI (){
		...
		BezierWaypoint.HandleStyle newHandleType = (BezierWaypoint.HandleStyle)EditorGUILayout.EnumPopup("Handle Type", (BezierWaypoint.HandleStyle)handleTypeProp.intValue);
		
		if(newHandleType != (BezierWaypoint.HandleStyle)handleTypeProp.intValue)
		{
	...
	void OnSceneGUI()
	{
		...
		var fmh_96_64_638378955087611293 = point.transform.rotation; Vector3 newPosition = Handles.FreeMoveHandle(point.position, HandleUtility.GetHandleSize(point.position)*0.2f, Vector3.zero, Handles.CubeHandleCap);
	...
	private static void HandleConnected(BezierWaypoint p){
		Handles.color = Color.cyan;
		
		var fmh_111_64_638378955087648965 = p.transform.rotation; Vector3 newGlobal1 = Handles.FreeMoveHandle(p.globalHandle1, HandleUtility.GetHandleSize(p.globalHandle1)*0.15f, Vector3.zero, Handles.SphereHandleCap);
		...
		var fmh_119_64_638378955087654266 = p.transform.rotation; Vector3 newGlobal2 = Handles.FreeMoveHandle(p.globalHandle2, HandleUtility.GetHandleSize(p.globalHandle2)*0.15f, Vector3.zero, Handles.SphereHandleCap);
	...
	private static void HandleBroken(BezierWaypoint p){
		Handles.color = Color.cyan;

		var fmh_131_64_638378955087659450 = Quaternion.identity; Vector3 newGlobal1 = Handles.FreeMoveHandle(p.globalHandle1, HandleUtility.GetHandleSize(p.globalHandle1)*0.15f, Vector3.zero, Handles.SphereHandleCap);
		var fmh_132_64_638378955087663784 = Quaternion.identity; Vector3 newGlobal2 = Handles.FreeMoveHandle(p.globalHandle2, HandleUtility.GetHandleSize(p.globalHandle2)*0.15f, Vector3.zero, Handles.SphereHandleCap);
	...
	private static void HandleAbsent(BezierWaypoint p)
	{
